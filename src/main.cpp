  ...
  h.onMessage([&pf,&map,&delta_t,&sensor_range,&sigma_pos,&sigma_landmark](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        vector<LandmarkObs_kd> noisy_observations_kd;
        ...
        for(int i = 0; i < x_sense.size(); i++){
          if (!pf.optimize){
            ...
            LandmarkObs_kd obs_kd;
        		obs_kd.dim[0] = x_sense[i];
				    obs_kd.dim[1] = y_sense[i];
				    noisy_observations_kd.push_back(obs_kd);
          }
        }
        ...
        if (!pf.optimize){
          ...
          pf.updateWeights_optimized(sensor_range, sigma_landmark, noisy_observations_kd, map);
        }
        pf.resample();
